...
import { 
  ...
  ShieldCheck, ArrowRight, Upload, Paperclip
} from "lucide-react";
...
export default function SafetyDpiRegistryPage() {
  ...
  const [uploadingRequest, setUploadingRequest] = useState<SafetyDpiAssignment | null>(null);
  ...
  const handleExecuteUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const docId = await uploadHRDocument(
        ...
        uploadFile,
        {
          title: `PV de remise EPI - ${emp?.displayName || 'Employé'} - ${uploadingRequest.dpiName}`,
          ...
          relatedId: uploadingRequest.assignmentId,
          isSensitive: false,
          status: "valid"
        },
        user.uid,
      ...
      await updateDpiAssignment(entityId, uploadingRequest.assignmentId, { reportDocumentId: docId }, user.uid);
      
      toast({ title: "PV enregistré", description: "Le document de remise a été lié à l'équipement." });
        ...
        <DialogContent className="sm:max-w-[450px] rounded-[2rem]">
          <DialogHeader>
            <DialogTitle className="text-xl font-black text-primary flex items-center gap-2">
              <Paperclip className="w-6 h-6" /> Joindre PV de remise
            </DialogTitle>
            <DialogDescription>
              Lier le procès-verbal signé pour : <span className="font-bold">{uploadingRequest?.dpiName}</span>.
            </DialogDescription>
          </DialogHeader>
                  ...
                  uploadFile ? "bg-green-50 border-green-200" : "bg-slate-50 border-slate-200 hover:bg-slate-100"
                   ...
                   {uploadFile ? (
                      ...
                      <>
                        ...
                        <p className="text-xs font-bold text-slate-600">Cliquez pour choisir le PV</p>
