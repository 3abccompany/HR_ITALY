...
export default function InterviewConfirmationPage() {
    ...
    async function load() {
      ...
      try {
        ...
        if (result.success && result.interview) {
          ...
          if (result.interview.confirmationStatus === "confirmed") setSuccess(true);
        } else {
  ...
  const handleConfirm = async () => {
    setProcessing(true);
    try {
      const result = await confirmInterviewAttendanceAction(token);
      if (result.success) {
        setSuccess(true);
      } else {
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <Card className="max-w-md w-full shadow-2xl border-none rounded-[2.5rem] overflow-hidden">
          ...
          <CardContent className="pt-16 pb-12 px-10 text-center space-y-6">
            <h1 className="text-3xl font-black text-slate-900">Présence confirmée !</h1>
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <main className="max-w-lg w-full space-y-8">
        <div className="text-center space-y-4">
          ...
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">Confirmer votre présence</h1>
          <p className="text-slate-500 font-medium leading-relaxed">
